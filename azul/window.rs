...
use glium::{
    ...
    glutin::{
        self, EventsLoop, ContextTrait, CreationError,
...
impl<T> Window<T> {
    ...
    pub(crate) fn new(
        ...
        if let Some(pos) = options.state.position {
            display.gl_window().window().set_position(winit_translate::translate_logical_position(pos));
        }
        ...
        if options.state.is_maximized && !options.state.is_fullscreen {
            ...
            display.gl_window().window().set_inner_size(winit_translate::translate_logical_size(options.state.size.get_inner_logical_size()));
        }
